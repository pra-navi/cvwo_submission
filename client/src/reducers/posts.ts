...
const postReducer = (state = { isLoading: true, posts: [], title: 'loading title...', currentPage: 1, numberOfPages: 1 }, action) => {
    switch (action.type) {
        ...
        case DISLIKE:
            return { ...state, posts: state.posts.map((post: { _id: string }) => post._id === action.payload._id ? action.payload : post) };
        case LIKE:
            return { ...state, posts: state.posts.map((post: { _id: string }) => post._id === action.payload._id ? action.payload : post) };
        case COMMENT:
            return { ...state, posts: state.posts.map((post: { _id: string }) => post._id === action.payload._id ? action.payload : post) };
        case DELETE:
            return { ...state, posts: state.posts.filter((post: { _id: string }) => post._id !== action.payload) };
        case UPDATE:
            return { ...state, posts: state.posts.map((post: { _id: string }) => post._id === action.payload._id ? action.payload : post) };
        case FETCH_ALL:
            return {
